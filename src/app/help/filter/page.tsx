...
const STEPS: Array<{
...
}> = [
  {
    ...
    body: (
      <>
        בסרגל הכלים לחצו על כפתור <HelpUiChip>סינון</HelpUiChip> (אייקון משפך). נפתח חלון
        מלמטה עם כל האפשרויות: שעות, רמת פחד, ממתקים, נגישות, שכונה, וסימונים שלכם (אהבתי /
        לא ביקרתי / דילגתי).
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <HelpUiChip>איפוס</HelpUiChip>) ולחצו <HelpUiChip>הצג תוצאות</HelpUiChip>.
      </>
    ),
  ...
  },
  {
    ...
    body: (
      <>
        אחרי שבחרתם קריטריונים לחצו <HelpUiChip>הצג תוצאות (מספר)</HelpUiChip> בתחתית החלון.
        המספר מראה כמה בתים עוברים את הסינון. ברשימה ובמסלול יופיעו{" "}
        <strong className="text-orange-200">רק</strong> אלה — שאר הבתים יישארו על המפה באפור.
        כפתור הסינון בסרגל נדלק בכתום כשיש סינון פעיל.
      </>
    ),
