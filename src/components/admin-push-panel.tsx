...
function Toggle({ on, onClick, disabled }: { on: boolean; onClick: () => void; disabled?: boolean }) {
  return (
    <span className="flex shrink-0 items-center gap-1.5">
      <span className={cn("text-sm", on ? "text-orange-200" : "text-violet-400")}>
        {on ? "פועל" : "כבוי"}
      </span>
      <button
        type="button"
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-sm text-violet-300">גם אתם תקבלו את ההתראה במכשיר הזה.</p>
        <Input
          value={title}
      ...
      <div className="space-y-2 rounded-xl bg-black/25 p-3">
        ...
        <div className="space-y-1.5">
          {templates.length === 0 ? (
            <p className="text-sm text-violet-400">טוענים תבניות…</p>
          ) : (
            templates.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {open ? (
                    <>
                      <p className="text-sm text-violet-400">{item.hint}</p>
                      <Input
                        value={draftTitle}
                        maxLength={80}
                        disabled={busy}
                    ...
                    <div className="rounded-md bg-[#0c0612]/80 px-2 py-1.5 ring-1 ring-orange-500/10">
                      ...
                      <p className="mt-0.5 whitespace-pre-line text-sm leading-snug text-violet-200">
                        {item.body}
                      </p>
                    </div>
                  )}
