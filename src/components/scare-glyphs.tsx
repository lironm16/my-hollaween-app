...
const PUMPKIN_GLYPH: Record<ScareLevel, string> = {
  mild: "/icons/scare-pumpkin-mild.png",
  medium: "/icons/scare-pumpkin-medium.png",
  spicy: "/icons/scare-pumpkin-spicy.png",
};

/** 5 — Jack-o’-lantern art from preview/scare-icon-options.png row 5. */
export function ScarePumpkin({ level = "mild" }: { level?: ScareLevel | "none" }) {
  const src = PUMPKIN_GLYPH[level === "none" ? "mild" : level];
  return (
    // eslint-disable-next-line @next/next/no-img-element
    <img src={src} alt="" aria-hidden className="block h-full w-full object-contain object-center" />
  );
